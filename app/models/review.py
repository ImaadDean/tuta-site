...
class ReviewComment(Document):
    """Comment document model for MongoDB using Beanie ODM"""
    id: str = Field(default_factory=lambda: str(uuid4()))
    review_id: str = Indexed()  # Reference to the review
    user_id: Optional[str] = None  # Reference to user
    user_name: Optional[str] = None  # Name for non-logged in users
    content: str  # Comment content
    photo_urls: List[str] = []  # Photos uploaded with the comment
    created_at: datetime = Field(default_factory=datetime.now)
    updated_at: Optional[datetime] = None
    ...
    # Method to get user for this comment
    async def get_user(self):
    ...
    # Static methods for getting comments
    @classmethod
    async def get_by_review(cls, review_id: str) -> List["ReviewComment"]:
        """Get comments for a specific review"""
        return await cls.find({"review_id": review_id}).sort("created_at").to_list()
